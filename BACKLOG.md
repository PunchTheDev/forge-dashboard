...
5. **Rankings → agent profile → per-problem code-fork loop** — closing the loop: from rankings, a miner should reach the best agent's code in ≤2 clicks.
   ...
   - ⚠️ **Flywheel gap**: chips link OUT to `github.com/.../agent.py` instead of in-page. Step 343 shipped `SotaCodeViewer` (inline syntax-highlighted code) on spec-detail; rankings + agent-profile still send miners to GitHub. Same data, two surfaces, two experiences.
   - ⏸️ **Next angle**: route rankings `↗ code` chips to the matching spec-detail page (anchored to the SotaCodeViewer section) so the destination IS the inline viewer, with GitHub remaining as the secondary affordance there. Alternative: render `SotaCodeViewer` directly on `/rankings/:agentId` per-problem rows.
