...
- Round-level SOTA chart (`RoundStandingsPanel`) — ● ● ● — was silently invisible on first paint: `useApi` only re-fired on `tick`, so the closure `() => round ? api.roundLeaderboard(...) : Promise.resolve(null)` resolved to `null` while `allRounds` was still loading and never re-fetched until the 60s interval. Fixed by extending `useApi` with a `deps` param and passing `[round?.id]` here (same patch also flows `[activeRoundId]` into the Rankings round-tab call site). Panel now renders within the first network round-trip — verified `/rounds/round_001/leaderboard 200` fires immediately and the table shows `#1 PunchTheDev · 1/15 · 0.967` on `/problems/round_001`. (step 361, `useApi.ts` + `App.tsx:1251`)
